...
PREFIX="./Xbox/${ARCH}D"
XBLIBS="xapilibd.lib xboxkrnl.lib vcompd.lib xbdm.lib"
...
if (isstaticlib) then
    ...
    for fname in *.def; do
        ...
        if (isintobin) then
            ...
            lib \/machine:ppcbe \/def:$fname \/out:$abspath
            popd
        	rm -fv $binpath/$outname.exp
        	pushd $libpath
        else
            lib \/machine:ppcbe \/def:$fname \/out:$outname.lib
            rm -fv $outname.exp
        fi
    done
    popd
